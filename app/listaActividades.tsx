...
export default function ListaActividades({ actividadesIniciales }: { actividadesIniciales: Actividad[] }) {
  ...
  const categorias = Array.from(
    new Set(actividadesIniciales.map((a) => a.categoria).filter(Boolean))
  );
  ...
  const actividadesFiltradas = actividadesIniciales.filter((act) => {
    ...
    const coincideCat = categoriaSel === '' || act.categoria === categoriaSel;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white p-4 sm:p-6 rounded-2xl shadow-xs border border-[#EBF2E8] flex flex-col sm:flex-row gap-4 items-center justify-between">
        {/* Input Buscador */}
        ...
        <div className="w-full sm:w-auto flex flex-wrap gap-2 items-center">
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all ${
              categoriaSel === ''
                ? 'bg-[#F48C2E] text-white shadow-xs'
          ...
          {categorias.map((cat) => (
            <button
              ...
              className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all ${
                categoriaSel === cat
                  ? 'bg-[#1FA4B6] text-white shadow-xs'
                  : 'bg-[#FAFAF7] text-[#6B5340] border border-[#EBF2E8] hover:bg-[#EBF2E8]'
              }`}
            >
              {cat}
          ...
          {actividadesFiltradas.map((act) => {
            ...
            return (
              <Link
                key={act.id}
                href={`/${act.slug}`}
                className="group bg-white rounded-2xl border border-[#EBF2E8] overflow-hidden shadow-xs hover:shadow-md hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between"
              >
                <div>
                  {/* Foto */}
                  <div className="relative h-48 w-full bg-[#EBF2E8] overflow-hidden">
                    ...
                    />
                    {act.categoria && (
                      <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-xs text-[#1FA4B6] text-xs font-bold px-2.5 py-1 rounded-lg shadow-xs">
                        {act.categoria}
                      </span>
                    )}
                  </div>
